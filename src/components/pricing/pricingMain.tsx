...
export default function PricingMain() {
	const plans = [
		...
		// },
		{
			...
			popular: true,
			features: [
				"Full course access", "3 Month Mighty Network access", "Exclusive resources"
			],
		},
	]
		...
		<MainWrapper className="border-y py-32 bg-zinc-50">
			<div className="container px-4 md:px-6">
				<div className="flex flex-col items-center justify-center space-y-4 text-center">
					<h1
						className="text-3xl font-bold tracking-tighter sm:text-4xl md:text-5xl lg:text-6xl/none">Pricing</h1>
					<p
				...
				<div className="flex flex-row gap-4 justify-center mt-12">
					{plans.map((plan) => (
						<Card key={plan.name + plan.period}
									className={`flex flex-col ${plan.popular ? "border-primary" : ""}`}>
							<CardHeader>
								<CardTitle className="text-xl">{plan.name}</CardTitle>
								{plan.popular && (
									<span
										className="px-2.5 py-0.5 text-xs font-semibold text-primary-foreground bg-primary rounded-full absolute top-4 right-4">
                    POPULAR
                  </span>
								)}
							</CardHeader>
							<CardContent className="flex-1">
								<div className="text-4xl font-bold">
									{plan.price}
									<span
										className="text-lg font-normal text-gray-500 dark:text-gray-400">{plan.period}</span>
								</div>
								{/*{plan.discount && <p*/}
								...
								<ul className="mt-4 space-y-2">
									{plan.features.map((feature) => (
										<li key={feature} className="flex items-center">
											...
											<span
												className="text-sm text-gray-500 dark:text-gray-400">{feature}</span>
										</li>
									))}
								</ul>
							</CardContent>
